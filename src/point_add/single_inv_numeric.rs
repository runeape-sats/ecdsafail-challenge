...
mod tests {
    ...
    fn half_gcd_tail_raw_rank_anf_stats(n: usize, p: u16) -> (usize, usize, usize) {
        use std::collections::{BTreeMap, BTreeSet};
        let size = 1usize << n;
        let mut by_key: BTreeMap<(i128, i128, i128, i128, String), BTreeSet<Vec<usize>>> =
            BTreeMap::new();
        let mut data = Vec::new();
        for x in 1..p {
            let mut u = p as i128;
        ...
        let ranked: BTreeMap<(i128, i128, i128, i128, String), Vec<Vec<usize>>> = by_key
            .into_iter()
    ...
    #[test]
    fn half_gcd_tail_raw_stream_is_self_delimiting_with_matrix_in_toys() {
        // Determinant compression reopens half-GCD payload size, but only if
        // the tail quotient stream can be consumed without a large boundary
        // parser.  Give the parser the full checkpoint matrix and only ask
        // whether raw concatenated binary tail quotients are self-delimiting.
        // If a rank bit is needed in any toy width, check whether that rank is
        // a dense function rather than a cheap local sidecar.
        let cases = [(8usize, 251u16), (10, 1021), (12, 4093), (14, 16381)];
        for &(n, p) in &cases {
            let (degree, density, max_mult) = half_gcd_tail_raw_rank_anf_stats(n, p);
            let table = 1usize << n;
            eprintln!(
                "half-GCD tail raw rank ANF: n={n}, max_mult={max_mult}, degree={degree}, density={density}/{table}"
            );
            if n == 14 {
    ...
    fn half_gcd_second_column_prefix_step_toy_cleans_history() {
        ...
        // `(b,d) -> (d,b-q*d)`, can copy out the useful residual/coefficient
        // values and semantically reverse to clean quotient digits.
        ...
        let rem_copy = b.alloc_qubits(REM_W);
        let coeff_copy = b.alloc_qubits(COEFF_W);
        ...
        for i in 0..REM_W {
            b.cx(rem[i], rem_copy[i]);
        }
        for i in 0..COEFF_W {
            b.cx(coeff_acc[i], coeff_copy[i]);
        }
        emit_toy_halfgcd_floor_inline_coeff_inverse_for_centered_test(
            &mut b,
            &rem,
            &divisor,
            &coeff_acc,
        ...
        let div_mask = (1u64 << DIV_W) - 1;
        let coeff_mask = (1u64 << COEFF_W) - 1;
        let mut classical_final_negative_cases = 0usize;
        for d in 1u64..(1u64 << DIV_W) {
            for u in 0u64..(d * (1u64 << DIGITS)).min(96) {
                for b0 in -5i64..=5i64 {
                    for d0 in -2i64..=2i64 {
                        let q = (u / d) as i64;
                        let expected_rem = u % d;
                        let expected_coeff = b0 - q * d0;
                        if u != 0 {
                            let (_, prefinal_rem, _) =
                                nonrestoring_prefinal_signed_digits_for_centered_test(
                                    U512::from(u),
                                    U512::from(d),
                                );
                            classical_final_negative_cases +=
                                (prefinal_rem.neg && !prefinal_rem.mag.is_zero()) as usize;
                        }
                        assert!(
                            (-(1i64 << (COEFF_W - 1))..(1i64 << (COEFF_W - 1)))
                                .contains(&expected_coeff),
                        ...
                        assert_eq!(rem_restored, u, "u was not restored u={u} d={d}");
                        assert_eq!(div_out, d, "v changed u={u} d={d}");
                        assert_eq!(
                            coeff_restored, b0,
                        ...
                        assert_eq!(
                            ...
                            0,
                            "digit history leaked u={u} d={d}"
                        );
                        assert_eq!(
                            sim.qubit(final_negative) & 1,
                            0,
                            "final_negative leaked u={u} d={d}"
                        );
                        assert_eq!(sim.global_phase() & 1, 0, "unexpected phase u={u} d={d}");
                    }
                }
            }
        }
        ...
        println!(
            "METRIC halfgcd_second_col_prefix_step_toy_final_negative_cases={classical_final_negative_cases}"
        );
        eprintln!(
            "half-GCD second-column prefix step toy: ccx={ccx}, peak={peak}, final_negative_cases={classical_final_negative_cases}, expected_ccx={expected_ccx}"
        );
        assert_eq!(ccx, expected_ccx, "half-GCD prefix step toy cost drifted");
        assert!(
            classical_final_negative_cases > 0,
            "toy fixture did not exercise final-negative correction"
        );
    }
    ...
    fn emit_barrel_left_shift_signed_nooverflow_for_plusminus(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
        kbits: &[super::super::QubitId],
        spill: &[super::super::QubitId],
    ...
    fn emit_barrel_left_shift_signed_nooverflow_inverse_for_plusminus(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
        kbits: &[super::super::QubitId],
        spill: &[super::super::QubitId],
    ...
    #[test]
    fn plusminus_signed_barrel_shift_roundtrip_is_phase_clean() {
        // Signed coefficient-lane counterpart to the unsigned denominator
        // barrel shift.  Valid inputs satisfy signed no-overflow for every
        // active stage; shifted-out sign bits are cleaned using the post-shift
        // sign bit.
        use sha3::digest::{ExtendableOutput, Update};
        const W: usize = 16;
        const KB: usize = 4;
        ...
        let spill = b.alloc_qubits(W);
        let start_left = b.ops.len();
        emit_barrel_left_shift_signed_nooverflow_for_plusminus(&mut b, &v, &kbits, &spill);
        let left_ccx = local_count_ccx_for_plusminus_cost(&b.ops[start_left..]);
        let start_inv = b.ops.len();
        emit_barrel_left_shift_signed_nooverflow_inverse_for_plusminus(&mut b, &v, &kbits, &spill);
        let inv_ccx = local_count_ccx_for_plusminus_cost(&b.ops[start_inv..]);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
    ...
    fn nonrestoring_floor_restoring_final_digits_for_centered_test(
        n: U512,
        d: U512,
    ) -> Vec<(bool, usize)> {
        assert!(!d.is_zero());
        ...
        if rem.neg && rem.mag > d {
            rem = signed_add_for_halfgcd_test(rem, smag_for_halfgcd_test(false, d << 1usize));
            q = signed_add_for_halfgcd_test(q, smag_for_halfgcd_test(true, U512::from(2u64)));
            digits.push((true, 1));
        } else if rem.neg && !rem.mag.is_zero() {
            rem = signed_add_for_halfgcd_test(rem, smag_for_halfgcd_test(false, d));
            q = signed_add_for_halfgcd_test(q, smag_for_halfgcd_test(true, U512::from(1u64)));
            digits.push((true, 0));
        ...
        assert!(
            !rem.neg && rem.mag < d,
            "restoring-final remainder escaped floor range n={n} d={d} top={top} rem={rem:?} q={q:?}"
        );
        assert!(
            !q.neg && q.mag == n / d,
            "restoring-final signed digits did not produce floor quotient"
        );
        digits
    }
    ...
    fn emit_active_fused_sign_controlled_addsub_digit_for_centered_test(
        b: &mut super::super::B,
        acc: &[super::super::QubitId],
        shifted_v: &[super::super::QubitId],
        active: super::super::QubitId,
        ...
        assert_eq!(acc.len(), shifted_v.len());
        let gated = b.alloc_qubits(shifted_v.len());
    ...
    fn emit_fused_sign_controlled_addsub_digit_opposite_for_centered_test(
        b: &mut super::super::B,
        acc: &[super::super::QubitId],
        shifted_v: &[super::super::QubitId],
        sign_hist: super::super::QubitId,
        ...
        b.x(sign_hist);
        emit_fused_sign_controlled_addsub_digit_for_centered_test(b, acc, shifted_v, sign_hist);
        b.x(sign_hist);
    }
    ...
    fn emit_shift_right_1_nooverflow_inverse_for_centered_test(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
    ) {
    ...
    fn emit_arithmetic_shift_right_1_exact_inverse_for_centered_test(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
    ) {
        assert!(v.len() >= 2);
        b.cx(v[v.len() - 2], v[v.len() - 1]);
        emit_shift_right_1_nooverflow_inverse_for_centered_test(b, v);
    }

    fn emit_controlled_shift_left_1_with_spill_for_centered_test(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
        ctrl: super::super::QubitId,
        spill: super::super::QubitId,
        ...
        assert!(!v.is_empty());
        for i in (0..v.len()).rev() {
            let hi = if i + 1 == v.len() { spill } else { v[i + 1] };
            super::super::cswap(b, ctrl, v[i], hi);
        }
    }

    fn emit_controlled_shift_left_1_with_spill_inverse_for_centered_test(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
        ctrl: super::super::QubitId,
        spill: super::super::QubitId,
    ...
    fn emit_toy_halfgcd_floor_inline_coeff_inverse_for_centered_test(
        ...
        for sh in 0..digit_hist.len() {
            emit_fused_sign_controlled_addsub_digit_opposite_for_centered_test(
                b,
                coeff_acc,
                &shifted_cv,
                digit_hist[sh],
        ...
        for i in (0..coeff_v.len() - top_shift).rev() {
            b.cx(coeff_v[i], shifted_cv[i + top_shift]);
        }
        for i in (0..divisor.len()).rev() {
            b.cx(divisor[i], shifted_v[i + top_shift]);
    ...
    fn emit_toy_direct_centered_nonrestoring_inline_coeff_restoring_final_for_centered_test(
        ...
        coeff_v: &[super::super::QubitId],
        digit_hist: &[super::super::QubitId],
        final_neg1: super::super::QubitId,
        final_neg2: super::super::QubitId,
        final_pos_ge: super::super::QubitId,
        final_lt_tmp: super::super::QubitId,
        q_neg: bool,
    ) {
        assert_eq!(digit_hist.len(), 6);
        assert_eq!(rem.len(), 11);
        ...
        let top_shift = digit_hist.len() - 1;
        assert!(coeff_v.len() > top_shift);
        let sign_false = b.alloc_qubit();
        let sign_true = b.alloc_qubit();
        b.x(sign_true);
        let coeff_sign_pos = if q_neg { sign_true } else { sign_false };
        let coeff_sign_neg = if q_neg { sign_false } else { sign_true };
        ...
        for sh in (1..digit_hist.len()).rev() {
            b.cx(rem[rem.len() - 1], digit_hist[sh]);
            emit_fused_sign_controlled_addsub_digit_for_centered_test(
                b,
                rem,
        ...
        b.cx(final_neg1, pos);
        b.x(pos);
        super::super::cmp_lt_into(b, rem, &shifted_v, final_lt_tmp);
        b.x(final_lt_tmp);
        b.ccx(pos, final_lt_tmp, final_pos_ge);
        b.x(final_lt_tmp);
        super::super::cmp_lt_into(b, rem, &shifted_v, final_lt_tmp);
        emit_active_fused_sign_controlled_addsub_digit_for_centered_test(
            b,
        ...
        super::super::sub_nbit_qq_fast(b, &half_v, rem);
        for i in (1..divisor.len()).rev() {
            b.cx(divisor[i], half_v[i - 1]);
        }
        b.free_vec(&half_v);
    ...
    fn emit_toy_direct_centered_restoring_final_single_selector_for_centered_test(
        b: &mut super::super::B,
        rem: &[super::super::QubitId],
        divisor: &[super::super::QubitId],
        coeff_acc: &[super::super::QubitId],
        ...
        let final_active = b.alloc_qubit();
        b.cx(final_neg1, final_active);
        b.cx(final_pos_ge, final_active);
        let coeff_final_sign = b.alloc_qubit();
        if q_neg {
            b.x(coeff_final_sign);
        }
        b.cx(final_neg1, coeff_final_sign);

        let shifted_v_spill = b.alloc_qubit();
        let shifted_cv_spill = b.alloc_qubit();
        emit_controlled_shift_left_1_with_spill_for_centered_test(
            b,
            &shifted_v,
            final_neg2,
            shifted_v_spill,
        ...
        emit_controlled_shift_left_1_with_spill_inverse_for_centered_test(
            b,
            &shifted_cv,
            final_neg2,
            shifted_cv_spill,
        ...
        b.free(shifted_cv_spill);
        b.free(shifted_v_spill);

        b.cx(final_neg1, coeff_final_sign);
        if q_neg {
            b.x(coeff_final_sign);
        }
        b.free(coeff_final_sign);
        b.cx(final_pos_ge, final_active);
        b.cx(final_neg1, final_active);
        b.free(final_active);
    ...
    fn emit_toy_direct_centered_restoring_final_single_selector_inverse_for_centered_test(
        b: &mut super::super::B,
        rem: &[super::super::QubitId],
        divisor: &[super::super::QubitId],
        coeff_acc: &[super::super::QubitId],
        ...
        super::super::add_nbit_qq_fast(b, &half_v, rem);
        for i in (1..divisor.len()).rev() {
            b.cx(divisor[i], half_v[i - 1]);
        }
        b.free_vec(&half_v);
        ...
        b.cx(final_neg1, final_active);
        b.cx(final_pos_ge, final_active);
        let inv_final_sign = b.alloc_qubit();
        b.x(inv_final_sign);
        b.cx(final_neg1, inv_final_sign);
        let coeff_final_sign_inv = b.alloc_qubit();
        if !q_neg {
            b.x(coeff_final_sign_inv);
        }
        b.cx(final_neg1, coeff_final_sign_inv);
        let shifted_v_spill = b.alloc_qubit();
        let shifted_cv_spill = b.alloc_qubit();
        emit_controlled_shift_left_1_with_spill_for_centered_test(
            b,
        ...
        b.free(coeff_final_sign_inv);
        b.cx(final_neg1, inv_final_sign);
        b.x(inv_final_sign);
        b.free(inv_final_sign);
        b.cx(final_pos_ge, final_active);
        b.cx(final_neg1, final_active);
        b.free(final_active);
    ...
    fn emit_toy_direct_centered_restoring_final_adjusted_step_fast_for_centered_test(
        b: &mut super::super::B,
        rem: &[super::super::QubitId],
        divisor: &[super::super::QubitId],
        coeff_acc: &[super::super::QubitId],
        ...
        b.free_vec(&half_v);
        emit_toy_direct_centered_nonrestoring_inline_coeff_restoring_final_for_centered_test(
            b,
            rem,
            divisor,
            coeff_acc,
    ...
    fn emit_toy_direct_centered_restoring_final_adjusted_step_fast_inverse_for_centered_test(
        b: &mut super::super::B,
        rem: &[super::super::QubitId],
        divisor: &[super::super::QubitId],
        coeff_acc: &[super::super::QubitId],
        ...
        let sign_true = b.alloc_qubit();
        b.x(sign_true);
        let coeff_sign_pos_inv = if q_neg { sign_false } else { sign_true };
        let coeff_sign_neg_inv = if q_neg { sign_true } else { sign_false };
    ...
    fn emit_twos_cneg_exact_for_centered_test(
        b: &mut super::super::B,
        v: &[super::super::QubitId],
        ctrl: super::super::QubitId,
    ) {
    ...
    fn twos_cneg_exact_cost_for_centered_test(width: usize) -> usize {
        let mut b = super::super::B::new();
        let v = b.alloc_qubits(width);
        let ctrl = b.alloc_qubit();
        let start = b.ops.len();
        emit_twos_cneg_exact_for_centered_test(&mut b, &v, ctrl);
        local_count_ccx_for_plusminus_cost(&b.ops[start..])
    }
    ...
    #[test]
    fn direct_centered_restoring_final_inline_coeff_toy_is_phase_clean() {
        // Avoid the late final_negative correction by making the last digit
        // restoring: negative remainders add one or two divisors, nonnegative
        // remainders subtract once only when still at least the divisor.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        for q_neg in [false, true] {
            ...
            let coeff_v = b.alloc_qubits(COEFF_W);
            let digit_hist = b.alloc_qubits(DIGITS);
            let final_neg1 = b.alloc_qubit();
            let final_neg2 = b.alloc_qubit();
            let final_pos_ge = b.alloc_qubit();
            let final_lt_tmp = b.alloc_qubit();
            let start = b.ops.len();
            emit_toy_direct_centered_nonrestoring_inline_coeff_restoring_final_for_centered_test(
                &mut b,
                &rem,
            ...
            let coeff_mask = (1u64 << COEFF_W) - 1;
            let div_mask = (1u64 << DIV_W) - 1;
            let mut final_neg2_cases = 0usize;
            let mut final_zero_cases = 0usize;
            for d in 1u64..(1u64 << DIV_W) {
                for n in 0u64..49u64 {
                    for cu0 in -5i64..=5i64 {
                        for cv0 in -2i64..=2i64 {
                            ...
                            let hist = get_slice_u512_pm(&sim, &digit_hist).as_limbs()[0];
                            let neg1 = sim.qubit(final_neg1) & 1;
                            let neg2 = sim.qubit(final_neg2) & 1;
                            let pos_ge = sim.qubit(final_pos_ge) & 1;
                            final_neg2_cases += neg2 as usize;
                            final_zero_cases += ((neg1 | neg2 | pos_ge) == 0) as usize;
                            let mut q = 0i64;
                            for sh in 1..DIGITS {
                                if ((hist >> sh) & 1) == 0 {
                                    q += 1i64 << sh;
                            ...
                            assert_eq!(
                                hist & 1,
                                0,
                                "unused digit_hist[0] changed n={n} d={d} q_neg={q_neg}"
                            );
                            assert_eq!(
                                sim.qubit(final_lt_tmp) & 1,
                                0,
                                "final comparator scratch leaked n={n} d={d} q_neg={q_neg}"
                            );
                            assert_eq!(
                                rem_out, expected_rem,
                                "centered remainder mismatch n={n} d={d} q_neg={q_neg}"
                            );
                            assert_eq!(
                                q, expected_q,
                                "quotient mismatch n={n} d={d} q_neg={q_neg}"
                            );
                            assert_eq!(
                                coeff_out, expected_coeff,
                                "coeff mismatch n={n} d={d} cu0={cu0} cv0={cv0} q_neg={q_neg}"
                            );
                            assert_eq!(
                                coeff_v_out, cv0,
                            ...
                            assert_eq!(
                                sim.global_phase() & 1,
                                0,
                                "unexpected phase n={n} d={d} cu0={cu0} cv0={cv0} q_neg={q_neg}"
                            );
                        }
                    }
                }
            ...
            println!("METRIC centered_direct_restoring_final_toy_{suffix}_ccx={ccx}");
            println!("METRIC centered_direct_restoring_final_toy_{suffix}_peak_q={peak}");
            println!(
                "METRIC centered_direct_restoring_final_toy_{suffix}_neg2_cases={final_neg2_cases}"
            );
            println!(
                "METRIC centered_direct_restoring_final_toy_{suffix}_zero_final_cases={final_zero_cases}"
            );
            eprintln!(
    ...
    #[test]
    fn direct_centered_restoring_final_single_selector_toy_prices_select1() {
        // Try to realize the optimistic select1 restoring-final ledger as a real
        // reversible step.  The last digit is encoded as one signed correction:
        // -2 uses a controlled one-bit shift, -1/+1 use the unshifted divisor,
        // and zero disables the final correction.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        for q_neg in [false, true] {
            ...
            let final_lt_tmp = b.alloc_qubit();
            let start = b.ops.len();
            emit_toy_direct_centered_restoring_final_single_selector_for_centered_test(
                &mut b,
                &rem,
                &divisor,
                &coeff_acc,
            ...
            println!(
                "METRIC centered_direct_restoring_final_single_selector_toy_{suffix}_zero_final_cases={final_zero_cases}"
            );
            eprintln!(
    ...
    #[test]
    fn direct_centered_restoring_final_single_selector_bennett_toy_cleans_history() {
        // The select1-shaped final digit still leaves the digit and branch
        // history live.  Check that copying out the useful outputs and reversing
        // the exact emitted step cleans every history bit.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        for q_neg in [false, true] {
            ...
            emit_toy_direct_centered_restoring_final_single_selector_inverse_for_centered_test(
                &mut b,
                &rem,
                &divisor,
                &coeff_acc,
                ...
                for n in 0u64..49u64 {
                    for cu0 in -5i64..=5i64 {
                        for cv0 in -2i64..=2i64 {
                            ...
                            assert_eq!(
                                coeff_restored, cu0,
                                "coefficient accumulator was not restored n={n} d={d} q_neg={q_neg}"
                            );
                            assert_eq!(
                                coeff_out, expected_coeff,
            ...
            let suffix = if q_neg { "qneg" } else { "qpos" };
            println!("METRIC centered_direct_restoring_final_single_selector_bennett_toy_{suffix}_ccx={ccx}");
            println!("METRIC centered_direct_restoring_final_single_selector_bennett_toy_{suffix}_peak_q={peak}");
            eprintln!(
    ...
    #[test]
    fn direct_centered_restoring_final_bennett_step_toy_cleans_history() {
        // The forward toy leaves quotient digits and final-selector flags as
        // history.  A production inversion can only use that shape if the step
        // can be copied out and then reversed to clean those histories.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        for q_neg in [false, true] {
            ...
            let final_lt_tmp = b.alloc_qubit();
            let start = b.ops.len();
            emit_toy_direct_centered_restoring_final_adjusted_step_fast_for_centered_test(
                &mut b,
                &rem,
                &divisor,
                &coeff_acc,
            ...
            emit_toy_direct_centered_restoring_final_adjusted_step_fast_inverse_for_centered_test(
                &mut b,
                &rem,
                &divisor,
                &coeff_acc,
    ...
    #[test]
    fn direct_centered_sign_normalized_direct_cneg_centered_step_toy_is_phase_dirty() {
        // The first sign-normalized budget used the cheap direct controlled
        // two's-complement negation.  In the centered inline context that is
        // basis-correct but phase-dirty, so the direct cneg budget cannot be
        // promoted to implementation as-is.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        emit_twos_cneg_direct_for_centered_test(&mut b, &coeff_acc, norm_negative);
        let ccx = local_count_ccx_for_plusminus_cost(&b.ops[start..]);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
            ...
            for n in 0u64..49u64 {
                for cu0 in -5i64..=5i64 {
                    for cv0 in -2i64..=2i64 {
                        ...
                        let expected_coeff = if expected_norm {
                            -expected_coeff_raw
                        } else {
                            expected_coeff_raw
                        };
                        ...
                        let mut hasher = sha3::Shake128::default();
                        hasher.update(b"direct-centered-signnorm-centered-step-toy-v1");
                        let mut xof = hasher.finalize_xof();
                        let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                        set_slice_u512_pm(&mut sim, &rem, U512::from(adjusted));
                        set_slice_u512_pm(&mut sim, &divisor, U512::from(d));
        ...
        let expected_ccx = DIGITS * ((REM_W - 1) + (COEFF_W - 1))
            ...
            + twos_cneg_direct_cost_for_centered_test(COEFF_W);
        println!("METRIC centered_direct_signnorm_centered_step_toy_ccx={ccx}");
        println!("METRIC centered_direct_signnorm_centered_step_toy_peak_q={peak}");
        println!("METRIC centered_direct_signnorm_centered_step_toy_norm_cases={normalized_cases}");
        println!("METRIC centered_direct_signnorm_centered_step_toy_phase_dirty_cases={phase_dirty_cases}");
        eprintln!(
    ...
    fn direct_centered_sign_normalized_exact_cneg_centered_step_toy_is_phase_clean() {
        ...
        // reality check needed before charging the exact-cneg budget.
        use sha3::digest::{ExtendableOutput, Update};
        ...
        b.cx(rem[REM_W - 1], norm_negative);
        emit_twos_cneg_exact_for_centered_test(&mut b, &rem, norm_negative);
        emit_twos_cneg_exact_for_centered_test(&mut b, &coeff_acc, norm_negative);
        let ccx = local_count_ccx_for_plusminus_cost(&b.ops[start..]);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
            ...
            for n in 0u64..49u64 {
                for cu0 in -5i64..=5i64 {
                    for cv0 in -2i64..=2i64 {
                        ...
                        let mut hasher = sha3::Shake128::default();
                        hasher.update(b"direct-centered-signnorm-exact-cneg-centered-step-toy-v1");
                        let mut xof = hasher.finalize_xof();
                        let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                        set_slice_u512_pm(&mut sim, &rem, U512::from(adjusted));
                        set_slice_u512_pm(&mut sim, &divisor, U512::from(d));
        ...
        let expected_ccx = DIGITS * ((REM_W - 1) + (COEFF_W - 1))
            + centered_final_half_fix_cost_for_centered_test(REM_W)
            + (2 * COEFF_W - 1)
            + twos_cneg_exact_cost_for_centered_test(REM_W)
            + twos_cneg_exact_cost_for_centered_test(COEFF_W);
        println!("METRIC centered_direct_signnorm_exact_cneg_centered_step_toy_ccx={ccx}");
        println!("METRIC centered_direct_signnorm_exact_cneg_centered_step_toy_peak_q={peak}");
        println!("METRIC centered_direct_signnorm_exact_cneg_centered_step_toy_norm_cases={normalized_cases}");
        eprintln!(
    ...
    #[test]
    fn direct_centered_nonrestoring_inline_coeff_negative_q_naive_toggle_is_phase_dirty() {
        // The signed Euclid recurrence can flip the quotient sign from the
        // denominator/remainder signs.  With q_neg=true the same magnitude digit
        // stream must update coefficients as cu -= (-q)*cv = cu + q*cv, while
        // leaving the magnitude-division remainder/history unchanged.  The
        // naive implementation below toggles the digit control around the same
        // fused add/sub primitive; it preserves basis-state semantics but is not
        // phase-clean on negative two's-complement coefficient addends.
        use sha3::digest::{ExtendableOutput, Update};
            ...
            for n in 0u64..49u64 {
                for cu0 in -5i64..=5i64 {
                    for cv0 in -2i64..=2i64 {
                        ...
                        let mut hasher = sha3::Shake128::default();
                        hasher.update(b"direct-centered-inline-coeff-negative-q-toy-v1");
                        let mut xof = hasher.finalize_xof();
                        let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                        set_slice_u512_pm(&mut sim, &rem, U512::from(adjusted));
                        set_slice_u512_pm(&mut sim, &divisor, U512::from(d));
        ...
        let expected_ccx = DIGITS * ((REM_W - 1) + (COEFF_W - 1))
            + centered_final_half_fix_cost_for_centered_test(REM_W)
            + (2 * COEFF_W - 1);
        println!("METRIC centered_direct_inline_coeff_negative_q_naive_ccx={ccx}");
        println!("METRIC centered_direct_inline_coeff_negative_q_naive_peak_q={peak}");
        println!("METRIC centered_direct_inline_coeff_negative_q_naive_phase_dirty_cases={phase_dirty_cases}");
        eprintln!(
    ...
    fn direct_centered_sign_normalized_inline_coeff_budget_probe() {
        ...
        let exact_cneg258 = exact_cneg_costs[258];
        ...
        let mut exact_norm_once_static = Vec::with_capacity(samples);
        let mut exact_norm_split_static = Vec::with_capacity(samples);
        let mut norm_counts = Vec::with_capacity(samples);
        ...
        let mut exact_norm_costs_static = Vec::with_capacity(samples);
        let mut exact_norm_rem_costs = Vec::with_capacity(samples);
        let mut exact_norm_coeff_costs = Vec::with_capacity(samples);
        let mut counts = Vec::with_capacity(samples);
        let mut digit_payloads = Vec::with_capacity(samples);
        let mut raw_digit_scratches = Vec::with_capacity(samples);
        let mut normflag_scratches = Vec::with_capacity(samples);
        ...
        let mut exact_norm_split_sum = 0i128;
        let mut norm_once_first64_sum = 0i128;
        let mut norm_split_first64_sum = 0i128;
        let mut exact_norm_once_first64_sum = 0i128;
        let mut exact_norm_split_first64_sum = 0i128;
        for sample_idx in 0..samples {
            let mut x = rand_u256(&mut rng);
            if x.is_zero() {
                x = U256::from(1u64);
            ...
            while !v.is_zero() {
                ...
                if !r_signed.mag.is_zero() {
                    let plus_u = qv + r_signed.mag;
                    let plus_q = (plus_u + (v >> 1usize)) / v;
                    let plus_valid = plus_u >= v && plus_q == q_direct;
                    let minus_valid = if qv >= r_signed.mag {
                        let minus_u = qv - r_signed.mag;
                        minus_u >= v && (minus_u + (v >> 1usize)) / v == q_direct
                    } else {
                        false
                    };
                ...
                if r_signed.neg && !r_signed.mag.is_zero() {
                    ...
                    exact_norm_coeff_cost += exact_cneg_costs[coeff_lane_width];
            ...
            let exact_norm_once = 642_716isize
                + 2 * (3 * coeff_width_cost
                    + exact_norm_coeff_cost
                    + exact_norm_rem_cost
                    + 2 * extraction_oneway) as isize;
            let exact_norm_split = 642_716isize
                + 2 * (3 * coeff_width_cost
                    + exact_norm_coeff_cost
                    + 2 * (extraction_oneway + exact_norm_rem_cost)) as isize;
            norm_once_sum += norm_once as i128;
            norm_split_sum += norm_split as i128;
            exact_norm_once_sum += exact_norm_once as i128;
            exact_norm_split_sum += exact_norm_split as i128;
            if sample_idx < 64 {
                norm_once_first64_sum += norm_once as i128;
                norm_split_first64_sum += norm_split as i128;
                exact_norm_once_first64_sum += exact_norm_once as i128;
                exact_norm_split_first64_sum += exact_norm_split as i128;
            }
            base_inline_3x.push(base);
            norm_once_static.push(norm_once);
            norm_split_static.push(norm_split);
            exact_norm_once_static.push(exact_norm_once);
            exact_norm_split_static.push(exact_norm_split);
            ...
            exact_norm_costs_static.push(exact_norm_coeff_cost + exact_norm_rem_cost);
            exact_norm_rem_costs.push(exact_norm_rem_cost);
            exact_norm_coeff_costs.push(exact_norm_coeff_cost);
            counts.push(count);
            digit_payloads.push(digit_payload);
            raw_digit_scratches.push(n + digit_payload);
            normflag_scratches.push(n + digit_payload + norm_count);
        ...
        exact_norm_once_static.sort_unstable();
        exact_norm_split_static.sort_unstable();
        norm_counts.sort_unstable();
        norm_costs_static.sort_unstable();
        norm_rem_costs.sort_unstable();
        norm_coeff_costs.sort_unstable();
        exact_norm_costs_static.sort_unstable();
        exact_norm_rem_costs.sort_unstable();
        exact_norm_coeff_costs.sort_unstable();
        counts.sort_unstable();
        digit_payloads.sort_unstable();
        raw_digit_scratches.sort_unstable();
        normflag_scratches.sort_unstable();
        ...
        let exact_norm_once_p99 = exact_norm_once_static[p99];
        let exact_norm_split_p99 = exact_norm_split_static[p99];
        let norm_count_p99 = norm_counts[p99];
        let norm_count_max = *norm_counts.last().unwrap();
        let count_p99 = counts[p99];
        let count_max = *counts.last().unwrap();
        let digit_payload_p99 = digit_payloads[p99];
        let digit_payload_max = *digit_payloads.last().unwrap();
        let raw_digit_scratch_p99 = raw_digit_scratches[p99];
        let normflag_scratch_p99 = normflag_scratches[p99];
        let normflag_scratch_max = *normflag_scratches.last().unwrap();
        let normflag_scratch_gap_google = normflag_scratch_p99 as isize - 663isize;
        let stepflag_scratch_p99 = stepflag_scratches[p99];
        let stepflag_scratch_max = *stepflag_scratches.last().unwrap();
        let stepflag_scratch_gap_google = stepflag_scratch_p99 as isize - 663isize;
        let ambiguous_norm_count_p99 = ambiguous_norm_counts[p99];
        ...
        let exact_norm_cost_p99 = exact_norm_costs_static[p99];
        let exact_norm_rem_p99 = exact_norm_rem_costs[p99];
        let exact_norm_coeff_p99 = exact_norm_coeff_costs[p99];
        let norm_once_mean = (norm_once_sum / samples as i128) as isize;
        let norm_split_mean = (norm_split_sum / samples as i128) as isize;
        let exact_norm_once_mean = (exact_norm_once_sum / samples as i128) as isize;
        let exact_norm_split_mean = (exact_norm_split_sum / samples as i128) as isize;
        let norm_once_first64 = (norm_once_first64_sum / 64i128) as isize;
        let norm_split_first64 = (norm_split_first64_sum / 64i128) as isize;
        let exact_norm_once_first64 = (exact_norm_once_first64_sum / 64i128) as isize;
        let exact_norm_split_first64 = (exact_norm_split_first64_sum / 64i128) as isize;
        let norm_once_gap = norm_once_p99 - 2_700_000isize;
        let norm_split_gap = norm_split_p99 - 2_700_000isize;
        let exact_norm_once_gap = exact_norm_once_p99 - 2_700_000isize;
        let exact_norm_split_gap = exact_norm_split_p99 - 2_700_000isize;
        let norm_once_mean_gap = norm_once_mean - 2_700_000isize;
        let norm_split_mean_gap = norm_split_mean - 2_700_000isize;
        let exact_norm_once_mean_gap = exact_norm_once_mean - 2_700_000isize;
        let exact_norm_split_mean_gap = exact_norm_split_mean - 2_700_000isize;
        let norm_once_first64_gap = norm_once_first64 - 2_700_000isize;
        let norm_split_first64_gap = norm_split_first64 - 2_700_000isize;
        let exact_norm_once_first64_gap = exact_norm_once_first64 - 2_700_000isize;
        let exact_norm_split_first64_gap = exact_norm_split_first64 - 2_700_000isize;
        println!("METRIC centered_direct_signnorm_cneg257={cneg257}");
        println!("METRIC centered_direct_signnorm_cneg258={cneg258}");
        println!("METRIC centered_direct_signnorm_exact_cneg257={exact_cneg257}");
        println!("METRIC centered_direct_signnorm_exact_cneg258={exact_cneg258}");
        ...
        println!("METRIC centered_direct_signnorm_euclid_count_p99={count_p99}");
        println!("METRIC centered_direct_signnorm_euclid_count_max={count_max}");
        println!("METRIC centered_direct_signnorm_digit_payload_p99={digit_payload_p99}");
        println!("METRIC centered_direct_signnorm_digit_payload_max={digit_payload_max}");
        println!("METRIC centered_direct_signnorm_raw_digit_scratch_p99={raw_digit_scratch_p99}");
        println!("METRIC centered_direct_signnorm_normflag_scratch_p99={normflag_scratch_p99}");
        println!("METRIC centered_direct_signnorm_normflag_scratch_max={normflag_scratch_max}");
        println!("METRIC centered_direct_signnorm_normflag_scratch_gap_google={normflag_scratch_gap_google}");
        ...
        println!("METRIC centered_direct_signnorm_exact_cost_p99={exact_norm_cost_p99}");
        println!("METRIC centered_direct_signnorm_exact_rem_cost_p99={exact_norm_rem_p99}");
        println!("METRIC centered_direct_signnorm_exact_coeff_cost_p99={exact_norm_coeff_p99}");
        println!("METRIC centered_direct_signnorm_exact_once_p99={exact_norm_once_p99}");
        println!("METRIC centered_direct_signnorm_exact_split_p99={exact_norm_split_p99}");
        println!("METRIC centered_direct_signnorm_exact_once_gap_to_2700k={exact_norm_once_gap}");
        println!("METRIC centered_direct_signnorm_exact_split_gap_to_2700k={exact_norm_split_gap}");
        ...
        println!("METRIC centered_direct_signnorm_exact_split_mean_gap_to_2700k={exact_norm_split_mean_gap}");
        println!("METRIC centered_direct_signnorm_exact_once_first64={exact_norm_once_first64}");
        println!("METRIC centered_direct_signnorm_exact_split_first64={exact_norm_split_first64}");
        ...
        assert!(
            exact_norm_split_mean_gap < 0,
            "sampled average exact-cneg split sign normalization stopped fitting the harness objective"
        );
        assert!(
            exact_norm_split_first64_gap < 0,
            "first-64 exact-cneg split sign normalization stopped fitting the harness objective"
        );
        assert!(
            raw_digit_scratch_p99 <= 663,
            "raw sign-normalized digit stream stopped fitting Google scratch before norm flags"
        );
        assert!(
            normflag_scratch_gap_google > 0,
            "normalization flags fit Google scratch without compression; promote signnorm implementation"
        );
        assert!(
            stepflag_scratch_gap_google > 0,
            "uncompressed normalization flags fit Google scratch; promote signnorm implementation"
        );
        assert!(
    ...
    #[test]
    fn direct_centered_restoring_final_digit_removes_late_fix_budget() {
        // The phase-clean positive-q toy and the phase-dirty negative-q toys
        // point at the late final_negative correction as the hard part.  Replace
        // the last non-restoring bit by a restoring ternary digit: before the
        // sh=0 update, add on a negative remainder, subtract only when a
        // nonnegative remainder is at least the divisor, and otherwise emit a
        // zero digit.  This keeps the same floor quotient without a separate
        // final-fix add or a final_negative coefficient correction.
        let p = SECP256K1_P;
        let samples = 32_768usize;
        let mut rng = 0x2800_d1ce_f1f0_0001u64;
        let n = 256usize;
        let mut one_x_pointadds = Vec::with_capacity(samples);
        let mut two_x_pointadds = Vec::with_capacity(samples);
        let mut three_x_pointadds = Vec::with_capacity(samples);
        let mut coeff_width_costs = Vec::with_capacity(samples);
        let mut digit_payloads = Vec::with_capacity(samples);
        let mut no_unit_final_counts = Vec::with_capacity(samples);
        let mut counts = Vec::with_capacity(samples);
        for _ in 0..samples {
            let mut x = rand_u256(&mut rng);
            if x.is_zero() {
            ...
            let mut coeff_u = smag_for_halfgcd_test(false, U512::ZERO);
            let mut coeff_v = smag_for_halfgcd_test(false, U512::from(1u64));
            let (mut digit_payload, mut digit_width_cost, mut count, mut no_unit_final_count) =
                (0usize, 0usize, 0usize, 0usize);
            let mut coeff_width_cost = 0usize;
            while !v.mag.is_zero() {
                let public_bound = direct_centered_public_width_bound_for_step(n, count);
            ...
            assert_eq!(
                coeff_mod.mul_mod(x, p),
                gcd_mod,
                "restoring-final inline coefficient is not the denominator inverse up to gcd sign"
            );
            let public_width_sum = (0..count)
                .map(|step| direct_centered_public_width_bound_for_step(n, step))
                .sum::<usize>();
            let inactive_positions_tapered = public_width_sum - digit_payload;
            let barrel_and_scan_tapered = public_width_sum * (8usize + 1usize);
            let final_select_1x = public_width_sum;
            let pointadd_for = |select_factor: usize| -> isize {
                let extraction_oneway = digit_width_cost
                    + barrel_and_scan_tapered
                    + select_factor * final_select_1x
                    + inactive_positions_tapered;
                642_716isize + 2 * (3 * coeff_width_cost + 2 * extraction_oneway) as isize
            };
            one_x_pointadds.push(pointadd_for(1));
            two_x_pointadds.push(pointadd_for(2));
            three_x_pointadds.push(pointadd_for(3));
            coeff_width_costs.push(coeff_width_cost);
            digit_payloads.push(digit_payload);
            no_unit_final_counts.push(no_unit_final_count);
            counts.push(count);
        }
        one_x_pointadds.sort_unstable();
        two_x_pointadds.sort_unstable();
        three_x_pointadds.sort_unstable();
        coeff_width_costs.sort_unstable();
        digit_payloads.sort_unstable();
        no_unit_final_counts.sort_unstable();
        counts.sort_unstable();
        let p99 = samples * 99 / 100;
        let one_x_p99 = one_x_pointadds[p99];
        let two_x_p99 = two_x_pointadds[p99];
        let three_x_p99 = three_x_pointadds[p99];
        let one_x_gap = one_x_p99 - 2_700_000isize;
        let two_x_gap = two_x_p99 - 2_700_000isize;
        let three_x_gap = three_x_p99 - 2_700_000isize;
        let coeff_width_p99 = coeff_width_costs[p99];
        let digit_payload_p99 = digit_payloads[p99];
        let no_unit_final_p99 = no_unit_final_counts[p99];
        let count_p99 = counts[p99];
        println!("METRIC centered_direct_restoring_final_samples={samples}");
        println!("METRIC centered_direct_restoring_final_coeff_width_p99={coeff_width_p99}");
        println!("METRIC centered_direct_restoring_final_digit_payload_p99={digit_payload_p99}");
        println!("METRIC centered_direct_restoring_final_no_unit_digits_p99={no_unit_final_p99}");
        println!("METRIC centered_direct_restoring_final_count_p99={count_p99}");
        println!("METRIC centered_direct_restoring_final_select1x_p99={one_x_p99}");
        println!("METRIC centered_direct_restoring_final_select2x_p99={two_x_p99}");
        println!("METRIC centered_direct_restoring_final_select3x_p99={three_x_p99}");
        println!("METRIC centered_direct_restoring_final_select1x_gap_to_2700k={one_x_gap}");
        println!("METRIC centered_direct_restoring_final_select2x_gap_to_2700k={two_x_gap}");
        println!("METRIC centered_direct_restoring_final_select3x_gap_to_2700k={three_x_gap}");
        eprintln!(
            "Direct-centered restoring-final digit budget: coeff_width_p99={coeff_width_p99}, digit_payload_p99={digit_payload_p99}, no_unit_final_p99={no_unit_final_p99}, count_p99={count_p99}, select1x={one_x_p99}, select2x={two_x_p99}, select3x={three_x_p99}, gaps=({one_x_gap},{two_x_gap},{three_x_gap})"
        );
        assert!(
            two_x_gap < 0,
            "restoring-final digit cannot afford a 2x-width final selector"
        );
    }
    ...
    #[test]
    fn direct_centered_restoring_final_coeff_decoder_average_gate() {
        // The p99 restoring-final ledger is deliberately conservative, but the
        // production objective is average executed Toffoli over 64 shots.  This
        // combines the restoring-final base model and the coefficient q-decoder
        // on the same traces, including a scan-free lower bound.
        const TARGET: f64 = 2_700_000.0;
        let p = SECP256K1_P;
        let samples = 8192usize;
        let mut rng = 0xd1ce_c0ef_dec0_a64u64;
        let n = 256usize;
        let exact_barrel_bits = 8usize;
        let mut base_select3 = Vec::with_capacity(samples);
        let mut aug_exact_select3 = Vec::with_capacity(samples);
        let mut aug_noscan_select1 = Vec::with_capacity(samples);
        let mut aug_noscan_select2 = Vec::with_capacity(samples);
        let mut aug_noscan_select3 = Vec::with_capacity(samples);
        let mut decoder_exact_rows = Vec::with_capacity(samples);
        let mut decoder_digit_rows = Vec::with_capacity(samples);
        let mut first64_base_select3 = 0isize;
        let mut first64_aug_exact_select3 = 0isize;
        let mut first64_aug_noscan_select1 = 0isize;
        let mut first64_aug_noscan_select2 = 0isize;
        let mut first64_aug_noscan_select3 = 0isize;
        ...
        for sample_idx in 0..samples {
            ...
            while !v.mag.is_zero() {
                ...
                let decoder_width = u512_bit_len_for_halfgcd_test(numer)
                    ...
                    .max(1);
                decoder_digit += decoder_digits.len() * decoder_width.saturating_sub(1);
                decoder_scan += decoder_width * (exact_barrel_bits + 1);
            ...
            let select2 = pointadd_for(2);
            let select3 = pointadd_for(3);
            let decoder_exact = (decoder_digit + decoder_scan) as isize;
            let decoder_digit = decoder_digit as isize;
            let aug_exact3 = select3 + 4 * decoder_exact;
            let aug_noscan1 = select1 + 4 * decoder_digit;
            let aug_noscan2 = select2 + 4 * decoder_digit;
            let aug_noscan3 = select3 + 4 * decoder_digit;

            if sample_idx < 64 {
                first64_base_select3 += select3;
                first64_aug_exact_select3 += aug_exact3;
                first64_aug_noscan_select1 += aug_noscan1;
                first64_aug_noscan_select2 += aug_noscan2;
                first64_aug_noscan_select3 += aug_noscan3;
            }
            base_select3.push(select3);
            aug_exact_select3.push(aug_exact3);
            aug_noscan_select1.push(aug_noscan1);
            aug_noscan_select2.push(aug_noscan2);
            aug_noscan_select3.push(aug_noscan3);
            decoder_exact_rows.push(decoder_exact);
            decoder_digit_rows.push(decoder_digit);
        }
        ...
        let base_mean = mean(&base_select3);
        let exact_mean = mean(&aug_exact_select3);
        let noscan1_mean = mean(&aug_noscan_select1);
        let noscan2_mean = mean(&aug_noscan_select2);
        let noscan3_mean = mean(&aug_noscan_select3);
        let decoder_exact_mean = mean(&decoder_exact_rows);
        let decoder_digit_mean = mean(&decoder_digit_rows);
        let base_first64 = first64_base_select3 as f64 / 64.0;
        let exact_first64 = first64_aug_exact_select3 as f64 / 64.0;
        let noscan1_first64 = first64_aug_noscan_select1 as f64 / 64.0;
        let noscan2_first64 = first64_aug_noscan_select2 as f64 / 64.0;
        let noscan3_first64 = first64_aug_noscan_select3 as f64 / 64.0;
        let base_p99 = p99_of(&mut base_select3);
        let exact_p99 = p99_of(&mut aug_exact_select3);
        let noscan1_p99 = p99_of(&mut aug_noscan_select1);
        let noscan2_p99 = p99_of(&mut aug_noscan_select2);
        let noscan3_p99 = p99_of(&mut aug_noscan_select3);
        let decoder_exact_p99 = p99_of(&mut decoder_exact_rows);
        let decoder_digit_p99 = p99_of(&mut decoder_digit_rows);
        println!("METRIC centered_direct_restoring_final_avg_samples={samples}");
        ...
        println!(
            "METRIC centered_direct_restoring_final_avg_aug_noscan_select3_gap_to_2700k={:.3}",
            noscan3_mean - TARGET
        );
        eprintln!(
            "Direct-centered restoring-final average gate: base3_mean={base_mean:.1}, exact3_mean={exact_mean:.1}, exact3_first64={exact_first64:.1}, noscan means=({noscan1_mean:.1},{noscan2_mean:.1},{noscan3_mean:.1}), noscan first64=({noscan1_first64:.1},{noscan2_first64:.1},{noscan3_first64:.1}), p99 exact/noscan3=({exact_p99},{noscan3_p99})"
        );
        assert!(
            exact_mean > TARGET && exact_first64 > TARGET,
            "average exact coefficient decoder now fits; promote restoring-final decoder implementation"
        );
        assert!(
            noscan3_mean < TARGET && noscan3_first64 < TARGET,
            "scan-free restoring-final decoder lower bound no longer reaches the average target"
        );
    }

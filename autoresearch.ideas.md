...
- **BY route currently dead for SOTA with existing primitives.** Compact denominator-pair history sink is state-good (50-bit sidecar, selector/history≈564 bits incl. signs), but raw per-bit denominator replay is gate-dead (compute+uncompute=2,008,160 CCX, peak=3372q, projected total≈3.79M). The last possible escape, selected fixed-matrix windows, is now invalidated by `selected_fixed_matrix_window_variable_coeff_lower_bound_kills_by`: one quantum-selected coefficient multiply lower bound is 18,598 CCX and one selected window lower bound is 148,784 CCX at 2123q, before coefficient lookup/signs/q/cleanup/history. This is >14× the ~10k/window target. Do not continue BY unless a new primitive makes quantum-selected small-coefficient row formation essentially constant-coefficient priced. Full-ratio remains gate-blocked (`h0` constmul okay; A-step dense; serial inverse proxy~7.31M). See `src/point_add/by_sota_architecture.md`.

...
- **Direct controlled-constant Solinas corrections:** `KAL_DIRECT_CONST_HALVE=1` is clean and gives `4,121,014 @ 2715q` by replacing the loaded 256q constant register in modular halve with `csub_nbit_const_direct_fast`; default now is `4,109,878 @ 2716q` after the r-small threshold update below. Standalone direct cadd/csub basis+phase tests pass. The add analogue (`KAL_DIRECT_CONST_DOUBLE=1`) fails at the aggressive 375 bulk prefix with `altseed_classical_total=1`, `altseed_phase_batches_total=2`, but is clean with `KAL_BULK3_ITERS=370` at `4,121,506 @ 2716q`. Both direct cadd+csub under `KAL_BULK3_ITERS=370` are clean at `4,130,602 @ 2715q`, `emitted_ops=29,250,534` (`-1.5M` emitted, `+20.7k` Toffoli vs new default). This is a tested env-gated low-emitted-op/qubit tradeoff, not a default optimization. Next low-qubit target after direct halve is `bk_step4` tmp/carry transient.
- **R-small no-correction threshold:** `R_SMALL_THRESHOLD` is now default 257. Env probes: 256 clean at `4,110,898`, 257 clean at `4,109,878`, 258 fails phase (also with `KAL_BULK3_ITERS=370`). Small exact win only (`-2,040` Toffoli vs prior default), but keep the known cliff in mind.
- **BY route currently dead for SOTA with existing primitives.** Compact denominator-pair history sink is state-good (50-bit sidecar, selector/history≈564 bits incl. signs), but raw per-bit denominator replay is gate-dead (compute+uncompute=2,008,160 CCX, peak=3372q, projected total≈3.79M). The last possible escape, selected fixed-matrix windows, is now invalidated by `selected_fixed_matrix_window_variable_coeff_lower_bound_kills_by`: one quantum-selected coefficient multiply lower bound is 18,598 CCX and one selected window lower bound is 148,784 CCX at 2123q, before coefficient lookup/signs/q/cleanup/history. This is >14× the ~10k/window target. This also warns against Kaliski-jump/windowed-matrix variants unless their selected row formation avoids quantum variable coefficients. Do not continue BY unless a new primitive makes quantum-selected small-coefficient row formation essentially constant-coefficient priced. Full-ratio remains gate-blocked (`h0` constmul okay; A-step dense; serial inverse proxy~7.31M). See `src/point_add/by_sota_architecture.md`.
...
- Best/default exact: **4,089,274 Toffoli @ 2713 qubits**, phase-robust (`R_SMALL_THRESHOLD=257`, pair1=404, pair2=401; hardened alt-seed checks clean).
...
- Gap: **1.389M Toffoli**, **1538 qubits** to low-qubit target.
...
- **Post-BY Strategy E slope-coordinate attempt**: new non-BY affine map validated in `single_inv_numeric.rs`: `m=dy/dx`, `Rx=m^2-dx-2Qx`, `Ry=-m*(Rx-Qx)-Qy`. `strategy_e_slope_coordinate_formula_passes_200` passes 200 random secp256k1 additions. Fast invalidation: current reversible primitives need product-clean/inverse-sized machinery for the required in-place variable multiply `(c,m)->(c,-m*c-Qy)`, so `strategy_e_slope_coordinate_budget_requires_new_inplace_variable_multiply` records current-known≈2,988,510 Toffoli vs ≈2,022,750 only if a schoolbook-like in-place variable multiply exists; needed saving≈965,760. Decision: algebra valid, current circuit route dead; only revive if a genuinely new phase-clean in-place variable multiply/divide primitive appears. First such primitive attempt, `destructive_montgomery_product_is_algebraically_promising_but_not_locally_reversible`, validates forward destructive Montgomery multiplication (`t=x*y*2^-n`) but kills local bit clearing: for p=251,a=173 an 8-bit reachable window poststate has 512 valid `(old_t,bits)` predecessors. Follow-up `destructive_montgomery_reverse_trellis_needs_field_sized_state` also kills the small-state global reverse-decoding rescue: frontiers saturate at `[0,2p)` (`n=8,10,12 -> 502,2042,8186`). Thus consumed multiplier bits need history/checkpoints or a nonlocal inverse; this is not the missing IMUL. Second primitive attempt, `mbuc_product_cleanup_phase_oracle_is_not_low_degree_on_toy_field`, kills cheap MBUC cleanup of the old multiplier: the required phase is a mask bit of `z/x mod p`, and for p=251 its ANF has degree 15/16 and density 32518/65536, so the hoped-for sparse/low-degree phase oracle is absent. Adjacent raw-product scratch kickmix is also not free: `raw_product_measurement_phase_is_dense_not_free_kickmix` checks phases `mask·(x*y)` when both inputs are preserved, and at toy n=10 sees all-bit mask degree 19/20 density 427812/1048576 and high-bit mask degree 19/20 density 120581/1048576. High product bits are dense carry functions, so measuring the 2n-bit product scratch is not the missing cheap IMUL. Third primitive attempt, Montgomery q-history MBUC, was briefly promising in the `(x,old_y)` frame (`montgomery_q_history_phase_growth_is_not_obviously_exponential_dense`: n=8 density 2440/65536, n=10 density 31684/1048576), but `montgomery_q_history_phase_in_output_frame_is_dense_dead` kills it for actual in-place multiply cleanup: in the surviving `(x,z)` frame the same q-history phase has degree 16/16 and density 31032/65536. Fourth attempt, `lambda_square_cleanup_would_require_dense_sqrt_phase`, kills preserving denominator data and recovering λ from `λ²=Rx+dx+2Qx`: canonical sqrt phase is dense/high-degree (`n=8,10,12` densities `126/256`, `502/1024`, `2072/4096`) and secp256k1 sqrt is an exponentiation. Coordinate-model escape check, `efficient_curve_model_transforms_need_missing_torsion`, kills base-field Montgomery/Edwards/Hessian rewrites for exact secp256k1: prime order has no rational 2- or 3-torsion (`order mod 2=1`, `order mod 3=1`), while those models require it and birational maps preserve torsion. Fifth/global MBUC attempt, `top_level_mbuc_of_old_point_requires_dense_point_subtraction_phase`, kills measuring the old affine input after computing R out-of-place: phase correction `P=R-Q` on toy curve has degree 15/16 and density 19540/65536, so generic top-level MBUC is just dense point-subtraction phase correction. Caveat checked by `curve_support_mbuc_phase_still_scales_not_constant_degree`: even when interpolation is only required on valid curve outputs, minimum degree grows (`n=4,6,8,10,12 -> 1,3,3,4,4`), matching the dimension-threshold extrapolation `d≈0.22n≈56` at secp256k1 rather than a constant-degree identity. Sequential MBUC also does not obviously save it: `sequential_old_coordinate_mbuc_still_has_growing_phase_degree` keeps old x live while measuring old y and still sees growing support degree (`n=4,6,8,10 -> 1,2,2,3`, dimension extrapolation ~49 at secp256k1).
...
- **Two-channel affine coefficient search reduced/mostly killed**: with transform `T=[[a,k],[dx,0]]`, target delta is `a(rF-r0)+k(sF-s0)`. If scratch `r` must end as known/freeable and we avoid unknown `a`, then `r0=rF=constant`, reducing exactly to the shifted-Y case. A one-pair preserve-x variant is also killed by `single_coefficient_pair_cannot_preserve_x_and_expose_quotient_by_constant_tag`: with `r0=ρ,s0=y+β`, outputs are `s=ρx` and `r=k*y+(ρa+βk)`, and sampled `(a,k,1)` are affine-non-collinear, so no constants make the contaminant known. The data-dependent `a(x)` cancellation escape is also blocked by `a_coefficient_cancellation_is_dense_on_toy_kaliski`: mask bits of `a(x)` are full-degree/near-half-density (`n=8 density 132/256`, `n=12 density 2094/4096`). Remaining hope requires making `r` itself an output register, adding a second coefficient channel, or finding a different triangular DIV transform.
...
- **Naive coefficient side-channel invalidated**: `KAL_TAGGED_DIV_COEFF_CHANNEL=1` carries `(lam,ty)` through live Kaliski controls, consumes `ty` to zero, and was already too wide/expensive even in the older passing run (4,672,021 Toffoli / 2,977q: it removes pair1's two schoolbook multiplications but adds per-iteration coeff cswaps+cmodadd+double and keeps the old inverse `(r,s)+m_hist`). Current retest under the hardened exact/alt-seed guard is worse: `altseed_phase_batches_total=1`, so treat this env path as phase-unsafe unless its tag phase is fully re-derived. New test `stored_a_and_m_bits_recover_branch_pair` confirms a branch-only scaffold can recover `add` from stored `a` plus existing `m` via `add=f&!(a xor m)`.
...
- **m_hist / self-cleaning Kaliski correction**: iter-START fingerprint gives `m_i`, but iter-END+available flags does not; direct persistent `m_hist` removal is blocked without a new self-cleaning Kaliski body or pebble recomputation. Stronger exactness check `exhaustive_toy_full_poststate_does_not_recover_forward_branch` shows that even full tagged post-state `(u,v,r,s,f)` plus reverse iteration index has branch collisions on exhaustive tiny fields (`n=4,5,6` conflicts `108,1200,5760`), correcting the earlier optimistic secp sample. Live post-state inspection alone is not a self-cleaning branch oracle. Direct branch regeneration from preserved `x` is also not a tiny oracle: `initial_x_to_branch_history_oracle_is_dense_on_toy_kaliski` gets branch-history parity degree/density `n=8: 8,116/256` and `n=12: 11,1976/4096`, so it is essentially rerunning Kaliski. Exact history compression has a ~256-bit information floor: `exact_branch_history_has_field_entropy_lower_bound` finds branch sequences injective on exhaustive toy fields and collision-free on 4096 secp samples, so raw 407-bit history can at best save ~150 bits by coding, not collapse to ~64 bits. Combined qubit floor `coefficient_transform_history_floor_misses_low_qubit_budget`: Google low-qubit scratch beyond tx/ty=663q, r-as-output coefficient DIV floor=768q, second-channel floor=1280q before flags/carries. Exact coefficient-transform Kaliski history is not a 1175q route. Poststate self-cleaning is now killed more directly: `secp_local_poststate_predecessor_branch_is_ambiguous` enumerates exact local inverse branches on reached secp tagged poststates and finds ambiguity in 7205/8140 = 88.5% of sampled steps (`hist=[0,935,1833,1779,3593]`). Approximate poststate self-cleaning is also not a small-exception route: `tagged_full_poststate_branch_ambiguity_is_not_a_rare_exception` finds ambiguous full-poststate transition occurrences rising to 228000/937500 = 24.32% by toy n=8 with nonzero tag s0=x+y. Retagging does not help: `changing_linear_x_tag_does_not_fix_poststate_branch_ambiguity` checks s0=y+beta*x and gets identical ambiguity fractions for nonzero beta.
Directly X-measuring affine slope after point-add is also not a cheap MBUC cleanup: `measuring_lambda_after_affine_add_still_needs_growing_degree_phase` corrects `λ=-(R_y+Q_y)/(R_x-Q_x)` on curve-supported outputs and sees min degrees `2,3,3,4,4` for toy n=4,6,8,10,12. Slope-pencil coordinate rewrite is blocked too: `pencil_slope_coordinate_needs_dense_root_choice_phase` shows the line-through-Q root-choice bit is sqrt-like/dense (`n=12 degree=11 density=2006/4096`).
- **Remaining SOTA route**: either (a) derive clean DIV / coefficient-transform cleanup, or (b) a jumped/windowed Kaliski that avoids quantum variable coefficients. Generic QROM-selected Kaliski matrices are now killed by `selected_matrix_variable_coeff_lower_bound_kills_hybrid_kaliski_windows`: row-formation-only lower bounds are 4.19M/3.77M/3.63M per invocation for t=4/8/16, before QROM/cleanup, versus the needed ~0.9M. Local arithmetic swaps are now <10% levers.
